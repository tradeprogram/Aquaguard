...
- 브랜치: `main` 하나. 지금까지 58개 커밋(2026-08-28 기준), 전부 push 완료 (최신 커밋 `21d188a`). `git log --oneline`으로 항상 최신 상태 다시 확인할 것 — 이 문서보다 git이 항상 더 정확하다. **주의**: 이 숫자는 매번 갱신되지 않으므로 참고값으로만 볼 것 — 실제 상태는 항상 `git log`로 확인한다.
...
- `store.py`: 인메모리 `AlertStore`. 승인 상태머신(`대기`/`승인`/`거부`/`권고중(escalation)`) — 2026-08-29에 자동승인을 삭제하고 escalation으로 교체했다(§5 Module O). 오탐(`오탐판정`)이면 escalation을 걸지 않는다. **관공서 모드 전용**이며, 시민 모드는 이 상태머신을 거치지 않고 곧바로 경보격상한다.
...
- **골든타임 시간슬라이더**: 구현됨(2026-09-20, 아래 "토사/침수 시각화" 참조). 39시간 프레임을 스크럽하면 위험영역이 `arrival_hour` 순으로 누적되고, 우리 탐지(09:00)·발송(09:05)·공식 경보(12:37)가 같은 줄에 찍혀 리드타임이 한 화면에서 비교된다.
- **카메라 조작**: 좌클릭 드래그=이동, 스크롤=줌(화면 중앙 기준 고정), 우클릭/Ctrl+드래그=회전·기울기. `maxPitch:70`.
- 테스트용 빠른 이동 버튼: 산청(AOI)/서울 강남/부산 해운대

**토사/침수 시각화 — 슬라이더 폐기, 시간축으로 교체 (2026-09-20)**:
> 원래 요청은 실제 CFD 캡처(FLOW-3D류)를 참조로 "토양은 빨간색, 물은 파란색, 실제
> 예측 양까지 정밀하게" 였다. 1차 버전(커밋 `4759d2a`)은 손으로 배치한 흐름 중심선
> (`DEBRIS_CENTERLINE_M`/`FLOOD_CENTERLINE_M`)에 깊이 슬라이더를 붙인 what-if
> 볼륨이었다. **이제 없다.** 화면에서 모형 산출물과 구분이 안 돼 "실제 예측값 아님"
> 배지를 달아도 오해를 불렀고, 무엇보다 UI만 보고는 지금 보는 게 언제의 예측인지
> 알 수가 없었다.

**지금 있는 것**:
- **침수 — Module B 실산출**: SFINCS 최대침수심 래스터를 `module_b_flood/fim.py`가
  0.3/1/2/5m 구간으로 폴리곤화(547개, 최심 p90 10.83m), `/alerts/{id}/geojson`이
  4326으로 재투영. 높이는 그 구간의 실제 침수심(`depth_p90_m`)이다.
  **시간축이 없다** — 최대 침수심 한 장뿐이라 번져가는 과정이 없고, UI도 그렇게 쓴다
  (`flood_is_max: true`).
- **산사태 — Module A 시간축 위험영역**: 트랙①의 `scripts/45`가 실측 강우로 시간마다
  구동해 남긴 `risk_landslide_index.json`(프레임 39시간 + 폴리곤별 `arrival_hour`).
  `/alerts/{id}/timeline`이 프레임과 위험영역을 4326으로 한 번에 내려주고, UI가
  `arrival_hour <= 현재시각`으로 걸러 누적 표시한다.
  **높이는 표시용 고정값**(`RISK_WALL_HEIGHT_M = 14`) — 토석류 runout 모형이 없어
  퇴적 깊이를 낼 수 없다(4차 지시서 P1). 침수(실측 수심)와 혼동되지 않게 범례에 명시.
- **시간 스크러버**: 화면 하단에 `7/19(토) 09:00` 형식의 날짜·요일·시각, 그 시각의
  시간강우/24h 누적/도달한 위험영역 수. 프레임별 시간강우 막대(클릭하면 그 시각으로
  점프), 범위 슬라이더, `▶ 시간 재생`(900ms/프레임, 끝에서 자동 정지).
  우리 탐지 09:00 / 발송 09:05 / 공식 경보 12:37을 같은 줄에 찍어 리드타임이 한
  화면에서 비교된다.
- **고립 판정(§7)이 실데이터로**: 예전에는 슬라이더가 만든 밴드를 hazard_polygon으로
  재사용했다. 이제 시간축 위험영역 + Module B 침수 폴리곤을 합쳐 `/isolation-check`에
  넘긴다 — 스크럽하면 끊긴 도로와 고립 건물이 그 시각 기준으로 다시 계산된다(600ms 디바운스).
- **지역별 상태 명시**: 서울 강남은 재해 모형이 없다. 빈 지도를 띄우는 대신
  "○ 없음 — 위험영역·침수심 래스터 / ● 있음 — 건물·도로 3D, 대피소, 경로, 고립 판정"을
  그 자리에 적고 시간 스크러버는 내린다(산청 시간축을 서울에 띄우면 그 지역 수치처럼 읽힘).

**지도 지역은 산청·강남 둘뿐 (2026-09-20)**:
> 서초는 강남과 똑같은 "모형 없는 지역"이라 화면에 새로 보여주는 게 없는데 정적 타일만
> 그만큼 더 들고 있었다. 서울 AOI를 강남구 total_bounds로 좁히고 그 바깥 타일을 지웠다.

- 소스마다 `bounds: AOI_BOUNDS[region]`이 걸려 있어 MapLibre는 범위 밖 타일을 **요청하지
  않는다** — bbox를 좁히면 그 바깥은 디스크에만 남는 죽은 파일이 된다. 실제로 시 전역
  (605km²) 시절 타일 411MB가 서빙되지 않은 채 남아 있었다(2026-09-05에 bbox만 좁히고
  타일은 안 지웠기 때문).
- `ui/public/{tiles,satellite}` 739MB → 286MB (서울분 14,728개 · 444MB 삭제).
- 산청 타일은 원래부터 bbox에 맞아 있었다(범위 밖 0.2MB) — 이 수치 자체가 타일 범위
  계산이 맞다는 검증이다.
- 벡터 타일은 z15/16에서 38개가 "없음"으로 나오는데 정상이다 — tippecanoe는 피처가 없는
  칸에 타일을 안 만들고, MapLibre는 벡터 404를 "여기 데이터 없음"으로 처리한다. 위성은
  래스터라 빠지면 흰 구멍이 되므로 bbox 사각형을 빈칸 없이 다 채운다(fetch_aoi_satellite.py).
- **범위를 다시 넓히려면** 타일을 재생성해야 한다: `python scripts/fetch_aoi_satellite.py
  --region seoul` + `node ui/scripts/build_vector_tiles.mjs seoul-buildings seoul-roads
  seoul-landcover`. 백엔드 노출자산(`aoi_buildings_seoul_5179.geojson`)은 여전히
  강남+서초를 담고 있으므로 그쪽은 건드릴 필요 없다.

**한계 / 다음 단계**:
1. **토사 퇴적 깊이가 없다** — 위험영역은 "여기가 위험하다"까지만 말하고 얼마나 쌓이는지는
   말하지 못한다. 트랙①의 runout 라우팅(USGS PWFDF-R 계열)이 들어오면 침수와 같은
   방식으로 실제 깊이를 높이에 쓸 수 있다.
2. **침수에 시간축이 없다** — SFINCS 시계열 산출이 생기면 `flood_is_max`를 내리고
   침수도 프레임에 맞춰 번지게 한다. 그 전까지는 있는 척하지 않는다.
3. `arrival_hour`는 "그 시각에 임계를 처음 넘었다"이지 "그때 붕괴했다"가 아니다 —
   화면 문구를 바꿀 때 이 구분을 흐리지 말 것.
4. 서울 AOI는 건물·도로·대피소가 다 있으므로, 위험영역 입력만 생기면 같은 화면이
   그대로 동작한다. 없는 것은 입력이지 기능이 아니다.
5. **(해결됨)** "산청은 건물이 거의 안 뜬다" — VWorld 건물통합정보 연동(§1.5, §2.2,
   커밋 `abcb95a`)으로 해결. 침수 볼륨이 실제 건물 478개와 정확히 겹친다.
...
> **구현 완료 (2026-09-20). 아래 명세는 착수 당시 기록이라 한 군데가 낡았다** —
> "슬라이더 값 기준 침수/토사 볼륨"으로 hazard_polygon을 만들라고 써 있지만, 그
> 슬라이더는 폐기됐다(§1.5 "토사/침수 시각화"). 지금은 Module A 시간축 위험영역 +
> Module B 침수 폴리곤을 합쳐 넘기고, 시간 스크러버를 움직이면 그 시각 기준으로
> 다시 계산된다. 캐싱·디바운스 전략(§7.3)은 그대로 유효하다.
...
## 11. 트랙② 현황 (대응로직·데이터통합, 나정우 / 2026-09-05)

트랙② 소유 모듈 4개(C·D·G·H)가 모두 구현됐다. Module E는 2026-08-27에 트랙④로
이관됐으므로 이 트랙 소유가 아니다.
...
| 모듈 | 상태 | 테스트 | 위치 |
|---|---|---:|---|
| C `module_c_urban_rule` | 완료·머지됨 | 187 | main |
| D `module_d_exposure_overlay` | 완료·머지됨 (+ 주용도 조인 배선) | 54 | main |
| H `module_h_citizen_verification` | 완료·머지됨 | 85 | main |
| G `module_g_damage_cost` | 완료·**미머지** | 89 | `track2/module-g` |
| (참고) O `module_o_orchestrator` | 트랙③ | 11 | main |

전체 **426 passed**. 네 모듈 모두 §4.2 표준 진입점 `run(input) -> dict`과 공통
봉투를 따르고, 어떤 입력에도 예외를 밖으로 던지지 않는다. 계약 밖 판정 근거는
전부 `explain()`으로 뺐다(§6.1 — `data`에 필드 추가는 4인 합의 사안).

### 11.2 실데이터 현황

- **건축물**: `data/precomputed/sancheong_buildings.geojson` 57,681건. **원본은
  EPSG:4326이다**(V-World 응답 그대로). 파일명에 좌표계 표기가 없어 `*_5179`
  계약 필드에 그대로 넣기 쉬우니 주의 — 실제 파이프라인은
  `data/vector/aoi_buildings_5179.geojson`(2,410건, 재투영본)을 쓴다.
- **건축물대장 주용도 조인**: 산청 건물의 **64.9%**(37,418/57,681)에 주용도가
  붙는다. 나머지 35.1%는 매핑 문제가 아니라 **데이터 부재**다(표제부에 대응
  레코드가 없는 농막·미등기·무허가). 배선 전에는 100%가 '미상'이었다.
  분류: 주거 27,824 / 상업 4,030 / 공업 2,601 / 농업 2,190 / 공공 773 / 기타 0.
  '기타'가 0인 것은 매핑이 산청 관측 32종을 전부 덮기 때문이다.
- **농경지(팜맵)**: `data/precomputed/sancheong_farmland_5179.geojson` 73,040건
  (원본 73,935건 - 비경지 895건). **밭 39,811 / 논 22,321 / 과수 5,784 / 시설 5,124.**
  원본 `AREA` 합과 지오메트리 계산 면적이 7,284.29 ha로 소수점까지 일치해
  EPSG:5179가 맞음을 확인했다(재투영 없음).
  > 커밋 `fbd853f7`의 메시지에는 논/밭이 뒤바뀌어 적혀 있다(콘솔 cp949 깨짐을
  > 잘못 옮긴 것). 파일·meta는 처음부터 정확하다. 히스토리는 재작성하지 않고
  > 여기에 정정값을 남긴다.
- **산청 D -> G 실행**(6km x 6km 위험영역): 노출 건물 1,152건(주거 648 / 미상 348 /
  상업 74 / 공업 58 / 농업 18 / 공공 6), 노출 농경지 624.5 ha ->
  **점추정 46.47억원, 구간 [46.47억, 60.42억]**.

### 11.3 정책 파일과 status 체계

임계값·단가·어휘는 코드가 아니라 JSON에만 산다. 근거를 주입한다는 것은 해당 행의
`source`를 채우고 `status`를 뒤집는 것이고, 파이썬 코드는 바뀌지 않는다.

| 위치 | 내용 |
|---|---|
| `policies/use_type_vocabulary.json` | **D와 G 공유** — 건축물 용도 7종과 주용도 매핑 |
| `policies/module_d.json` | 점 버퍼, min_risk_prob, 속성키, 주용도 조인 설정 |
| `policies/module_g.json` | 단가 고시 2건, 미상 주거비율, 최소 불확실성 폭 |
| `module_c_urban_rule/rulesets/v1_kma_mois.json` | 강우 절단점, 극한호우 기준 |
| `module_h_citizen_verification/policies/v1_default.json` | 집계 가중치·임계 |
| `docs/sources/` | 단가 고시 PDF 2건(G 정책 행이 경로로 가리킴) |

C와 H의 정책이 패키지 안에 있는 이유는 공유 대상이 없어서다. D와 G는 use_type
어휘를 공유해야 해서 최상위 `policies/`를 쓴다(한쪽 패키지에 두면 다른 쪽이 그
패키지에 의존해 §4.2 모듈 독립성이 깨진다).

**status 어휘** — 근거의 강도를 구분한다:

| status | 뜻 |
|---|---|
| `CONFIRMED` | 1차 자료로 확인, 계약 입력만으로 완전 재현 가능 |
| `CONFIRMED_COMPONENT` | 공식 기준의 일부만 적용(나머지 조건이 계약에 없음) |
| `DERIVED` | 공식 기준에서 우리가 환산·산출 |
| `POLICY_CONFIRMED_DIRECTION` | 행정 방향의 근거일 뿐 수치의 직접 출처는 아님 |
| `TEAM_DECISION` | 찾을 근거가 없어 팀이 정함 |
| `ASSUMPTION` | 명시적 가정 |
| `PLACEHOLDER` | 아직 근거를 못 찾음 — 조사 대상 |

`PLACEHOLDER`와 `TEAM_DECISION`을 구분하는 게 핵심이다. 전자는 찾으면 나오고
후자는 찾아도 안 나온다 — 후속 조치가 다르다.

### 11.4 주요 ASSUMPTION (발표·심사에서 방어해야 할 지점)

| # | 가정 | 영향 범위 | 해소 조건 |
|---|---|---|---|
| 1 | **주용도 조인이 필지 단위**(동 단위 아님) | 산청 필지의 35%(10,967개)에 표제부가 2건 이상 — 대표 선택 규칙(주건축물 우선, 동률 시 연면적 최대)이 실제로 작동 | 표제부에 `bd_mgt_sn` 일련번호[19:25] 대응 필드가 생기면 해소(현재 없음) |
| 2 | **주거 1동 = 1세대** | G 점추정의 건물 항목 전체. 공동주택 미반영이라 **과소추정** | 표제부 `hhldCnt`를 수집 파이프라인에 추가(필지 단위 배분 규칙 필요) |
| 3 | **점 좌표 100m 버퍼** | Module A가 폴리곤을 안 내놔서 O가 점을 넘길 때 D가 원으로 부풀림. 적용되면 항상 `degraded` + warning | 아젠다 1번(A 계약에 위험 폴리곤 필드 추가) 해결 시 폴백 전용으로 전락 |
| 4 | **shrinkage k=9** (H) | `confidence_adjustment` 전체. 계약 예시(6건→0.12)를 재현하도록 **역산한 값**이지 검증된 신뢰도 모델이 아님 | 실제 파일럿 응답 데이터로 재보정 |
| 5 | **최소 불확실성 폭 30%** (G) | '미상' 0건일 때 폭 0 구간을 막는 하한. 근거 없음 | 실측 분포 확보 시 교체 |

그 밖에 `drainage_factor`/`known_risk_factor`(C), `unknown_use_type_housing_ratio
0.744`(G, 산청 실측이라 **다른 지역에 그대로 쓰면 안 됨**)도 확인 대상이다.

#### PLACEHOLDER 잔여 목록 (2026-09-05 기준 11행)

`PLACEHOLDER`는 "아직 근거를 못 찾음"이다. `TEAM_DECISION`(찾아도 안 나옴)과 구분해서
읽을 것 — 아래는 전부 **조사하면 나올 수 있거나, 데이터로 재보정할 수 있는** 값이다.
`v0_placeholder` 룰셋(Module C의 근거 주입 이전 기준선)은 비교용 보존본이라 제외했다.

| 모듈 | 파일 | 행 id | 현재 값 | 왜 미확정인가 | 해소 조건 |
|---|---|---|---|---|---|
| C | `module_c_urban_rule/rulesets/v1_kma_mois.json` | `drainage_factor` | low 1.30 / medium 1.00 / high 0.85 | 배수능력 등급별 가중에 대응하는 공식 기준을 못 찾음 | 행안부·국토부의 지하차도 배수능력 기준 확보. 없으면 `TEAM_DECISION`으로 내리는 것도 방법 |
| C | 〃 | `known_risk_factor` | 1.15 | 지정 위험 지하차도 가중의 공식 출처 없음 | 〃 |
| D | `policies/module_d.json` | `point_buffer_radius_m` | enabled, 100m | Module A가 폴리곤 대신 점만 출력해서 D가 원으로 부풀림. 산사태 도달거리 자릿수 가정일 뿐 | **아젠다 1번** 해결(A 계약에 위험 폴리곤 필드 추가) 시 이 행 자체가 폴백 전용으로 전락 |
| D | 〃 | `building_id_keys` | `bd_mgt_sn` 등 7개 | **실측으로 이미 해소됨** — VWorld가 쓰는 실제 키명이 `bd_mgt_sn`이고 2,410건 전부 채워짐(99.97% 유니크) | status를 `CONFIRMED`로 올리면 됨. 남겨둔 건 다른 데이터 소스가 붙을 때를 대비한 탐색 목록이기 때문 |
| D | 〃 | `use_type_keys` | `mainPurpsCdNm` 등 6개 | note가 "실호출로 확인 전"이라고 적혀 있으나 **2026-09-04 probe로 확인됨**(응답에 `mainPurpsCdNm` 존재). note가 낡았다 | note 갱신 + status 상향 검토 |
| H | `module_h_citizen_verification/policies/v1_default.json` | `shrinkage_k` | 9.0 | **계약 예시 역산값**. 검증된 신뢰도 모델이 아님 | 실제 파일럿 응답 데이터로 재보정. 계약 예시가 바뀌면 이 값도 재검토 |
| H | 〃 | `confirm_threshold` | 가중 긍정 ≥ 3.0 | 근거 없는 초기값 | 파일럿에서 오경보율·미탐율 확인 후 조정 |
| H | 〃 | `false_positive_threshold` | 2건 이상 & 비율 0.5 이상 | 근거 없는 초기값(1건으로 못 뒤집게 두 조건을 함께 건 것은 의도적) | 〃 |
| H | 〃 | `full_weight_min_reports` | 3 | §7이 "응답 다수/소수"만 말하고 숫자를 안 줌 | §7 폴백표에 수치를 넣거나(아젠다 3번) 파일럿으로 결정 |
| H | 〃 | `valid_window_min` | 60분 | 근거 없는 초기값 | 실제 신고 도착 시간 분포 확보 시 재산출 |
| G | `policies/module_g.json` | `min_upper_band_pct` | 0.3 | 폭 0인 구간을 막기 위한 하한. 근거 없음 | 피해액 실측 분포 확보 시 교체 |

**가장 방어가 어려운 것은 H의 `shrinkage_k`다.** 나머지는 "근거를 아직 못 찾았다"인데
이건 **계약 예시 숫자에 맞춰 역산한 값**이라, 심사에서 "이 신뢰도 보정은 어떻게 나온
값인가"를 물으면 답이 "예시를 재현하려고 정했다"가 된다. 파일럿 응답 데이터 확보가
트랙② 잔여 과제 중 우선순위가 가장 높다.

### 11.5 미결 아젠다

`TRACK2_CONTRACT_AGENDA.md`에 10건이 있다. **1·2·7·8번은 2026-09-04 4인 합의로
해결**됐고(수범이 `contracts/` 반영), 남은 것은:

- **3번** §7 폴백 계층 표에 C·D·G·H 행 추가 (문서 반영 대기)
- **4번** 최상위 `policies/` 디렉토리 추가 (보고 사항, §8 트리에 없음)
- **5번** Module O가 Module C를 호출하지 않음 → **수범이 `62140123`에서 연결 완료**,
  확인만 남음
- **6번** 출처 확보가 남은 임계값 (트랙② 자체 과제)
- **9번** Module G 계약 예시가 자기 output을 못 만듦(약 64배) — 단순 예시 교체가
  아니라 12.5억의 근거 확인이 먼저다
- **10번** Module O의 `unit_cost_table_ref` 하드코딩 교체

### 11.6 다른 트랙에 대한 요청

**수범(트랙③)**

1. **농경지 AOI 클립본을 `data/vector/`에 커밋해달라.** 건물에 대해 이미 하신
   방식(`aoi_buildings_5179.geojson` 1.1MB) 그대로다. 팜맵 원본
   `sancheong_farmland_5179.geojson`은 75.6MB이고 `.gitignore` 대상이라 배포
   서버에 없어서, `exposure_layers.farmland_parcels()`가 계속 "농경지 레이어
   미확보" 경고와 함께 빈 컬렉션을 돌려준다. 클립본이 생기면 그 경고가 사라지고
   D의 `exposed_farmland_ha`가 배포 환경에서도 실값을 낸다.
2. **`orchestrator.py`의 `unit_cost_table_ref` 문자열 교체.** 지금
   `"재해연보_2024_원단위"`를 하드코딩해 넘기는데 G가 쓰는 표는 2026년 고시
   2건(`module_g_v1`)이라 상시 불일치다. G는 요청한 표를 쓴 척하지 않고 warning +
   `basis_citation`에 실제 표를 밝히지만, 호출부 문자열이 사실과 다르다.
   (아젠다 10번)
3. **UI의 피해액 라벨을 "노출 자산 기준 하한"으로.** G의 단가는 *복구비 지원*
   단가 고시라 실제 경제적 손실이 아니라 그 하한이다. 또 점추정은 `risk_prob`를
   곱하지 않은 **조건부 피해액**("침수되면 얼마")이지 기대손실이 아니다.
   `basis_citation`에 이미 명시했고 `explain()`에 위험도 가중값도 함께 내보내니,
   화면에서 "예상 피해액"으로만 읽히지 않게 해달라.
4. **원본 파일명에 좌표계 표기 제안.** `data/precomputed/*_buildings.geojson`은
   EPSG:4326인데 파일명에 표기가 없다. `*_buildings_4326.geojson`처럼 접미어를
   붙이면 `*_5179` 계약 필드에 잘못 넣는 사고를 막을 수 있다(내가 실제로 한 번
...
6. **Module C의 `alert_level` 표시에 "규칙 기반 - 예측 모델 아님" 배지를 붙여달라.**
   §6이 명시적으로 요구하는 사항이다("Module C는 신뢰도 배지를 A/B와 다르게 표시").
   C는 강우강도를 임계값과 비교하는 규칙 엔진이고 학습된 모델이 아닌데, 화면에서
   A/B의 확률값과 나란히 놓이면 같은 성격으로 읽힌다. 심사에서 "이 경보는 모델이
   낸 건가요"라는 질문이 나오면 배지 하나로 답이 끝난다.
   근거는 이미 모듈이 내보내고 있다 - C의 `explain()`이 `is_model: False`와
   적용된 절단점·계수·룰셋 출처를 전부 들고 나온다.

7. **C/D/G/H의 `explain()` 출력을 Provenance 4단계 배지에 연결하는 방안을 협의하자.**
   §6.1의 `MODEL`/`ASSUMPTION` 두 단계를 트랙② 모듈이 이미 기계적으로 판별할 수 있는
   형태로 내보내고 있는데, UI가 아직 안 읽는다.

   | 배지 | 트랙② 모듈이 내보내는 근거 |
   |---|---|
   | `MODEL` | 네 모듈 모두 `explain()`에 `is_model: False`를 낸다 - C는 규칙, D는 공간연산, G는 산식, H는 집계이지 학습된 모델이 아니다 |
   | `ASSUMPTION` | D `use_type_join.is_assumption`(필지 단위 조인), D `buffer_is_assumption`(점 100m 버퍼), G `references_not_in_estimate`(점추정 미반영 참고값), 각 정책 행의 `status`(`ASSUMPTION`/`PLACEHOLDER`) |

   협의가 필요한 지점은 **어느 수준에서 배지를 달 것인가**다. 화면 요소 하나에
   배지 하나를 다는 구조라면, 예컨대 "예상 피해액 46.47억"에는 `ASSUMPTION`이
   붙어야 한다(1동=1세대, 필지 단위 조인, 최소 불확실성 폭 30%가 모두 얽혀 있다).
   반대로 "노출 건물 1,152건"은 공간연산 결과라 성격이 다르다. 배지 매핑 규칙을
   트랙③이 정하면 트랙②가 `explain()` 필드를 그 규칙에 맞춰 정리하겠다.

**민석(트랙①)** — 아젠다 1번이 풀리면(Module A가 위험 폴리곤을 출력) D의 점 버퍼
가정(위 ASSUMPTION 3)이 폴백 전용으로 내려간다. 현재는 A가 점만 주므로 D가 항상
`degraded`를 낸다.

**공통** — `fetch_aoi_data.py`의 `merge_region_layer`가 서울(652,026건)에서
MemoryError로 죽는다. V-World 크롤 자체는 118/118 타일 성공하므로 재크롤은 필요
없고 병합만 스트리밍으로 고치면 된다. 트랙②는 우회로만 만들었고
(`fetch_building_use_types.py`가 타일 캐시를 직접 스트리밍) 그 파일은 손대지
않았다 — 서울 AOI가 UI에서 비어 있다면 원인이 이것일 수 있다.
