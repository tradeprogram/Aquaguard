...
- **`module_e.example.json`/`module_e.schema.json` 소유자가 트랙②→트랙④로 바뀌었다.** 네이버/카카오 길찾기 API + 고립마을 탐지가 더해지면서 필드 확장이 필요할 것(차량/도보 이동수단 분리 등) — 제안 스키마는 별도 정리돼 있으나 **아직 이 폴더에 반영 안 됨**, 실제로 바꾸기 전에 4인 합의부터.
- **`module_v.example.json`/`module_v.schema.json`은 신규(2026-08-29, 심층 경쟁기술 리서치 반영)** — 소유 트랙①. Sentinel-1 predicted-vs-observed 검증 엔진, 상세는 [ARCHITECTURE.md §5 Module V](../ARCHITECTURE.md).
