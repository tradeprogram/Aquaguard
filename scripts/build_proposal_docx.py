...
class Doc:
    ...
    def ui_slot(self, caption, *, how=None, height_cm=7.4):
        """UI 화면 사진이 들어갈 자리. 본문 흐름 안에 빈 상자로 표시한다.

        지도 화면은 WebGL이 필요해 생성 환경에서 캡처할 수 없다. 자동으로 채우는
        대신 자리와 "무엇을 찍어야 하는지"를 명시해, 사진을 받는 즉시 그 자리에
        끼워 넣을 수 있게 한다. 상자를 비워 두지 않고 안내문을 넣는 이유는,
        빈 칸이면 넣는 사람이 어느 화면인지 다시 물어봐야 하기 때문이다.
        """
        FIG_NO["n"] += 1
        tbl = self.d.add_table(rows=1, cols=1)
        tbl.style = "Table Grid"
        tbl.alignment = WD_TABLE_ALIGNMENT.CENTER
        cell = tbl.rows[0].cells[0]
        cell.width = Cm(15.6)
        tbl.rows[0].height = Cm(height_cm)
        _shade(cell, "F8FAFC")

        mark = cell.paragraphs[0]
        mark.alignment = WD_ALIGN_PARAGRAPH.CENTER
        mark.paragraph_format.space_before = Pt(height_cm * 8)
        _run(mark, "<<이곳에 삽입>>", bold=True, size=13, color=ACCENT)

        if how:
            guide = cell.add_paragraph()
            guide.alignment = WD_ALIGN_PARAGRAPH.CENTER
            _run(guide, how, size=9, color=MUTED, italic=True)
...
def write(D: Doc) -> None:
    ...
    D.table(
        ...
        [
            ...
            ("대상 지역", "경남 산청군 전역(790km², 주 사례) · 서울 강남구(확장성 검증)"),
    ...
    D.p("우리나라의 집중호우는 짧은 시간에 대량의 강수가 좁은 지역에 쏟아지는 형태로 바뀌고 "
        ...
        "도시화는 이 구조를 키웁니다 — 불투수면이 늘면 빗물이 땅으로 스미지 못하고 지표를 따라 "
        "빠르게 흘러 저지대·지하차도·도로의 배수능력을 넘어섭니다.")
    ...
    D.ui_slot("아쿠아가드 메인 화면 — 산청군 전역 조감. 브이월드 건물·도로 실데이터와 지형 위에 "
              "모듈 산출이 레이어로 올라간다",
              how="돋보기 → \"산청군 전체\" 버튼을 눌러 군 전역이 보이는 상태")
    ...
    D.ui_slot("Module B — 불어난 강물. 시간이 지나며 하천이 범람해 주변 농경지와 마을로 번진다. "
              "얕은 가장자리는 남색, 깊은 물길로 갈수록 시안→연두→노랑으로 침수심이 색으로 표시된다",
              how="시간축 ▶ 재생 → 2025.7.19 14:00에서 정지, 하천 구간이 넓게 보이도록 확대")

    D.ui_slot("Module B — 침수심 근접. 깊이는 높이가 아니라 색으로 표현하며, 원본 50m 격자를 "
              "등수심선을 따라 다듬어 그린다(원본 대비 IoU 0.930 · 면적 −0.2%)",
              how="깊은 물길이 노랗게 타는 지점을 최대한 확대")

    D.ui_slot("Module C·E — 물에 잠겨 통행이 끊긴 도로(빨강). 도로망 그래프에서 침수 영역과 겹치는 "
              "링크를 제거한 결과이며(침수 반영 1,956개 링크), 이 판정이 곧 고립 탐지의 입력이 된다",
              how="고립마을 위험 → “침수 반영” 실행 후 빨간 도로 구간이 보이도록 확대")

    D.ui_slot("Module E — 고립마을(마젠타). 위험 도로를 제거했을 때 104개 대피소 어디로도 경로가 "
              "남지 않는 건물 군집. 침수 반영 시 2,445동 · 68개 군집",
              how="같은 화면에서 마젠타 군집이 여러 개 보이도록 축소")

    D.ui_slot("Module E — 대피소(초록)와 실도로 대피 경로. 네이버 Directions로 조회한 실제 도로 "
              "폴리라인(302개 절점)이며 직선 근사가 아니다. 차량 14.3분 · 도보 166.8분",
              how="대피소 찾기 → 경로 선택 후 지도에 경로선이 그려진 상태")

    D.ui_slot("Module A — 산사태 위험영역. 주황 P≥0.5(대응 시작 기준) 안에 빨강 P≥0.7이 들어 있고, "
              "실제 폴리곤이 20~75m 조각이라 축소 시에도 보이도록 위치를 점으로 함께 표시한다",
              how="시간축에서 ⤢ 위험영역 버튼 클릭 → 주황·빨강 표시가 보이는 상태")

    D.ui_slot("Module D — 노출자산 판정의 바탕이 되는 건물·도로 실데이터. 건물은 브이월드 "
              "건물통합정보(층수×3m 근사 높이), 도로는 국가교통정보센터 표준노드링크이며 "
              "교량·고가차도는 지면에서 띄운 데크로 그린다",
              how="마을 하나를 확대하고 우클릭 드래그로 기울여 3D 건물이 드러나는 각도")
    ...
    D.ui_slot("시간축 비교 ① — 초기(2025.7.18 06:00). 다음 그림과 같은 화각이므로 나란히 놓으면 "
              "재해가 전개되는 과정이 그대로 드러난다",
              how="스크러버를 7/18 06:00에 두고 촬영. 카메라는 이후 움직이지 말 것")

    D.ui_slot("시간축 비교 ② — 첨두(2025.7.19 14:00). 같은 화각에서 침수 460→891ha, "
              "위험영역 14→390곳으로 늘어난다",
              how="카메라를 움직이지 말고 스크러버만 7/19 14:00으로 옮겨 촬영")
    ...
    D.ui_slot("시민 모드 — 우리동네 위험현황. 현재 위치 기준 위험 요약과 행동 지침",
              how="시민 모드 → “우리동네 위험현황” 실행 후 패널이 모두 뜬 상태")

    D.ui_slot("시민 모드 — 대피소 찾기. 대피소 목록과 각 대피소까지의 실도로 경로·소요시간",
              how="“대피소 찾기” 패널이 열리고 목록이 채워진 상태")

    D.ui_slot("시민 모드 — 고립마을 위험. 침수로 진입로가 끊겨 대피소에 닿지 못하는 구역 안내",
              how="“고립마을 위험” 패널이 결과를 표시한 상태")
    ...
    D.ui_slot("관공서 모드 — 경보 패키지 전체. 모듈별 산출과 provenance 배지가 함께 표시된 화면",
              how="관공서 모드로 전환 후 경보 패키지 패널 전체가 보이도록 촬영")

    D.ui_slot("관공서 모드 — 노출자산·피해비용 상세. 818동·102.5ha와 고시 단가 근거, "
              "가정 칩이 함께 붙은 상태", how="노출자산/피해비용 패널을 펼친 상태")

    D.ui_slot("관공서 모드 — 고립·통행불가 판정 결과. 군집 수, 고립 건물 수, 제외 사유 경고문이 "
              "함께 표시된다", how="고립 분석 결과 패널의 경고문까지 보이도록 촬영")

    D.ui_slot("관공서 모드 — 경보 초안. 승인 전까지 권고 상태이며 발령은 사람이 누른다",
              how="경보 초안이 생성된 상태의 패널")
    ...
    D.ui_slot("AI 재난 상담 챗봇 — 현재 경보 패키지를 컨텍스트로 받아 답변한다. "
              "답변의 수치는 화면 패널의 값과 같은 출처다",
              how="챗봇 패널을 열고 “지금 대피해야 하나요” 등 질문·답변이 모두 보이는 상태")
