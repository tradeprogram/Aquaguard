...
LDAPS 예보 강우는 아래 규격으로 받는다(검증 완료).
  nwp_vars_down.php?nwp=l015&sub=unis&vars=ncpc&tmfc=YYYYMMDDHH(UTC)&ef=N&dataType=TEXT
  - sub=unis 필수. 누락하면 없는 경로가 만들어져 file not exist 가 뜬다
  - vars 는 4글자 코드다. ncpcp(5글자)는 거부된다 — nwp_grib_guidance.pdf p21~22 의
    ${VAR} 열 참조. 강수 중 값이 오는 것은 ncpc(대규모강수)·lspr(강수율)
  - 국지 보유 2012-05~. "2026-03-31 까지" 안내는 UM 생산종료일이다
자동화: scripts/46_ldaps_forecast.py (격자 지리참조 포함).
상세 docs/handoff/TRACK1_REPLY.md §3.
"""
