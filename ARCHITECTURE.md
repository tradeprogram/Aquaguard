...
4. 위험도가 임계치를 넘고 시민 역검증(Module H)으로 신뢰도가 보강되면, 관(지자체 재난상황실)과 시민에게 곧바로 경보를 전파한다(§5 Module O — 골든타임 상태머신). 시민 직접배포 앱이라는 취지에 맞춰 지자체 담당자 승인 게이트는 두지 않는다(2026-08-29 결정, §5 Module O 참조).
...
산불로 훼손된 사면 위에 첫 집중호우가 떨어지는 순간부터, 산사태→하천범람→고립까지 이어지는 재해 사슬을 몇 시간 전에 읽어내고, 대피소·경로·피해규모까지 자동으로 계산해 관(官)과 시민에게 곧바로 전파하는 에이전트 — 시민 직접배포 앱이라 지자체 담당자의 승인 게이트 없이 시민 역검증(Module H)만으로 신뢰도를 보강한다.
...
- **독창성 축 3 — 판단 지연 자체를 겨냥한 2종 장치**(§5 Module H, Module O/UI): ① 시민 신고 역검증 루프(예측 신호를 시민이 즉시 확인해주는 채널 — 관 담당자 승인 게이트 없이 이 신뢰도 보강만으로 곧바로 경보격상) ② What-if 예측 시뮬레이터(가상 강수 시나리오를 심사위원이 직접 조작). (2026-08-29: 지자체 담당자 "원클릭 승인" 게이트는 시민 직접배포 취지·법적 리스크를 이유로 핵심 아키텍처에서 제외 — §5 Module O 참조.)
...
**상태머신 7단계**: 관측 → 예측 → 1차권고 → 시민 역검증(Module H, 병렬) → 경보격상 → 주민전파 → 개별대피
...
> **2026-08-29 변경**: 지자체 담당자 승인 게이트("원클릭 승인")를 핵심 아키텍처에서 제외했다. 시민 직접배포 앱이라는 취지와 안 맞고(관공서 워크플로우가 아니라 일반 시민이 쓰는 앱), 법적 책임소재 리스크도 있다는 판단. 시민 역검증(Module H)이 신뢰도를 보강하면 곧바로 경보격상·주민전파로 넘어간다 — 사람이 매 건 버튼을 눌러야 하는 단계가 없다. `approval_status`/`POST /approve/{alert_id}`/`auto_approve_timeout_min`는 코드(`module_o_orchestrator/store.py`, `ApprovePanel.tsx`, `/approve` 라우트)에는 아직 남아있지만 **더 이상 이 프로젝트의 핵심 제품 방향이 아니다** — 지자체 상황실용 B2G 버전이 따로 필요해지면 그때 재활용.
...
9. ~~원클릭 승인의 `auto_approve_timeout_min` 기본값 조정~~ — 2026-08-29 승인 게이트 자체를 핵심 아키텍처에서 제외해 더 이상 해당 없음
...
원클릭 승인 화면(`/approve`, `ApprovePanel.tsx`)은 2026-08-29부로 핵심 아키텍처에서 제외됐다 — 만들 필요 없음. 시민 역검증(Module H)이 신뢰도를 보강하면 곧바로 경보격상·주민전파로 넘어가는 흐름을 구현할 것.
