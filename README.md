...
가장 심각했던 곳은 경남 산청군 — 7/19 하루 동안 산사태 266건, 13명 사망·1명 실종. **진짜 실패 지점은 예측 실패가 아니라 전파 지연**이다: 산림청은 이미 7/17에 대피를 권고했고, 산사태 경보 격상은 12:37이었다(복수 보도로 확인) — 이미 있던 신호가 관(官)의 판단·전파 단계에서 여러 시간 샜다. **("7/19 08:00부터 주민 신고가 빗발쳤다"는 정확한 기준시각은 아직 1차 행정기록으로 확인 못함 — 산청군·산림청 공문 및 재난문자 원자료로 확정이 필요하다.)**
...
> **방법론 확정(2026-08-29)**: `landslide_prob`은 순수 ML이 아니라 Infinite Slope 기반 FoS(안전율, TRIGRS 원리)를 먼저 계산한 값이어야 한다 — 전문은 [ARCHITECTURE.md §5 Module A](ARCHITECTURE.md#module-a--산사태-예측-module_a_landslide). 물리 baseline을 먼저 두는 이유는 FoS가 "경사 34°, 포화도 증가, FoS 1.08→0.91" 같은 설명 가능한 근거를 그대로 화면에 노출할 수 있기 때문이다.
