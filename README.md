...
가장 심각했던 곳은 경남 산청군 — 7/19 하루 동안 산사태 266건, 13명 사망·1명 실종. **진짜 실패 지점은 예측 실패가 아니라 전파 지연**이다: 산림청은 이미 7/17에 대피를 권고했고, 산사태 경보 격상은 12:37이었다(복수 보도로 확인) — 이미 있던 신호가 관(官)의 판단·전파 단계에서 여러 시간 샜다. **("7/19 08:00부터 주민 신고가 빗발쳤다"는 정확한 기준시각은 아직 1차 행정기록으로 확인 못함 — 발표·제출 전 산청군/산림청/재난문자 원자료로 확정할 것, [HANDOFF.md §9](HANDOFF.md) 참조.)**
...
4. 위험도가 임계치를 넘고 시민 역검증으로 신뢰도가 보강되면, 관(지자체 재난상황실)과 시민에게 곧바로 경보 전파 (Module O — 골든타임 상태머신). 시민 직접배포 앱 취지에 맞춰 지자체 담당자 승인 게이트는 두지 않음(2026-08-29 결정)
5. 산청 실제 타임라인(신고 08:00, 경보 12:37)과 이 시스템이 있었다면 나왔을 타임라인을 비교해 "몇 시간 몇 분을 벌 수 있었는가"를 숫자로 증명 (데모 시나리오, §8)

### 공모전 트랙

**서비스 개발 부문** (2026-08-28 리서치로 확정, 상세는 [§15](#15-공모전-수상-전략-2026-08-28-리서치-반영) 참조). 이전에는 "아이디어/프로토타입 부문"으로 적어뒀었는데, 구현 자산(3D GIS·오케스트레이터·전국 행정경계 검색 등)이 이미 아이디어 부문에 넣기엔 과할 정도로 많고, 재난 대응 B2G 서비스라는 성격상 아이디어 본선의 "시장성 30점"보다 서비스 본선의 "완성도·확장성·유용성 75점"이 훨씬 유리하다고 판단해 정정. 데이터 스택·모듈 계약이 자세한 이유도 신뢰도를 받치는 인프라이기 때문 — 다만 **지금부터는 새 기능보다 실데이터·검증·사용자 실증에 개발 리소스의 80%를 쓰는 게 맞다.**
...
> **한 문장 포지셔닝** (2026-08-28 리서치 반영): 아쿠아가드는 새로운 산사태 위험지도가 아니다. 기존 산불·강우·지형·하천 신호를 하나의 재해연쇄로 연결해 위험도가 임계치를 넘은 순간 "어떤 도로가 끊기고, 어떤 마을이 고립되며, 누구를 어느 대피소로 보내야 하는지"를 하나의 의사결정 패키지로 생성하고, 실제 재난에서 그 의사결정이 몇 분 빨라질 수 있었는지를 검증하는 시스템이다.

- **독창성 축 1 — 재해연쇄**: `f(dNBR, Δt)`로 화재→산사태→하천범람 사슬을 정량 모델링 (산불 자체는 실시간 예측 대상이 아닌 정적 이력값). **주의**: 산불 이력이 산사태 위험을 높인다는 인과관계 자체는 이미 알려진 현상이다 — 산림청 강우반영 산사태위험도가 이미 산불피해지를 반영하고, 미국 USGS도 post-fire debris flow 확률·부피를 burn severity 기반으로 운영한다([HANDOFF.md §9](HANDOFF.md) 참조). 그래서 이 축의 진짜 독창점은 "산불→산사태 연결"이 아니라 **화재부터 하천범람·고립·의사결정까지 전체 사슬을 하나로 엮고, 그 사슬이 만든 골든타임을 실측으로 검증**한다는 것 — 축 2·4와 함께 봐야 의미가 있다.
- **독창성 축 2 — 골든타임 격차**: "예측 정확도"가 아니라 "관의 판단·전파 지연"을 자동화로 우회
- **독창성 축 3 — 판단 지연 자체를 겨냥한 2종 장치**: ① 시민 신고 역검증 루프(Module H — 관 담당자 승인 게이트 없이 이 신뢰도 보강만으로 곧바로 경보격상) ② What-if 예측 시뮬레이터(가상 강수 시나리오를 심사위원이 직접 조작). (2026-08-29: 지자체 담당자 "원클릭 승인" 게이트는 시민 직접배포 취지·법적 리스크를 이유로 제외)
...
**포지셔닝**: "KIGAM보다 정확한 모델"이 아니라 "KIGAM급 모델이 있어도 못 막은 전파 지연을 우회하는 시스템". TODO: `A_max` 등급값은 2025.3 산불 11개지역 자체 dNBR 산출 후 사후검증으로 보정. **발표·심사 답변에서 이 수치를 "검증된 한국형 계수"라고 말하지 말 것** — 사후검증 전까지는 문헌값을 가져다 쓴 가정치다.
...
**상태머신 7단계**: 관측 → 예측 → 1차권고 → 시민 역검증(H, 병렬) → 경보격상 → 주민전파 → 개별대피
...
> **2026-08-29 변경**: 지자체 담당자 승인 게이트("원클릭 승인")를 핵심 아키텍처에서 제외했다 — 시민 직접배포 앱 취지와 안 맞고 법적 책임소재 리스크도 있다는 판단. 시민 역검증(Module H)이 신뢰도를 보강하면 곧바로 경보격상·주민전파로 넘어간다. `approval_status`/`POST /approve/{alert_id}`는 코드(`ApprovePanel.tsx`, `/approve` 라우트)엔 아직 남아있지만 더 이상 핵심 제품 방향 아님 — 지자체용 B2G 버전이 필요해지면 재활용.
...
7. ~~원클릭 승인 `auto_approve_timeout_min` 기본값 조정~~ — 2026-08-29 승인 게이트 자체를 핵심 아키텍처에서 제외해 더 이상 해당 없음
...
- **트랙③**: `module_o_orchestrator`, `ui/`(대시보드+채팅+경보 전파+3D) — **대부분 이미 완성돼 있음**, 먼저 읽고 실행해볼 것. A~H를 전부 `contracts/`의 example.json으로 목업 호출하는 파이프라인부터 굴려보고 실제 모듈로 하나씩 교체. 원클릭 승인은 2026-08-29부로 핵심 아키텍처에서 제외됨(시민 직접배포 취지) — 만들 필요 없음
...
## 15. 공모전 수상 전략 (2026-08-28 리서치 반영)

외부 리서치(ChatGPT 심층 리서치, 분석기준일 2026.8.28)로 공모전 공식 배점·역대 수상작 패턴·현재 GitHub 구현 상태를 교차 분석한 결과를 반영했다. 원본은 팀 로컬에 있고, 결론만 이 문서에 정리한다 — 상세 실행계획은 [HANDOFF.md §9](HANDOFF.md).

### 공식 심사 배점

| 단계 | 항목 | 배점 | 전략적 의미 |
|---|---|---|---|
| 서류 | 기획성/중복성/적합성 | Pass 여부 | 단순 솔루션·제품 소개는 탈락 가능 |
| 예선 | 기획성 | 30 | 사회문제·재난대응 논리, 가장 큰 단일 점수 |
| 예선 | 활용성 | 20 | 공식 지원 플랫폼 + AI·클라우드·데이터·API 활용 증거 |
| 예선 | 유용성 | 20 | 시민·기업·정부가 실제 어떤 결정을 더 잘하게 되는가 |
| 예선 | 협력성 | 15 | 팀 외부 주체(민간·공공기관·학교)와 실제 협력 여부 |
| 예선 | 충실성 | 15 | 논리·도표·범례·출처·기획서 품질 |
| 본선 | 기획성 | 15 | 문제와 해결 구조 |
| 본선 | 완성도 | 25 | 기술·기능 구현 및 **배포** 완성도 |
| 본선 | 확장성 | 25 | 다른 지역·재난·운영 환경으로 확대 가능한가 |
| 본선 | 유용성 | 25 | 예상·실제 **사용자 수와 만족도** |
| 본선 | 발표 | 10 | 발표·자료 완성도 |

**핵심**: 예선(80점이 기획성+활용성+협력성+충실성 = 기술 완성도가 독립 배점으로 없음)과 본선(75점이 완성도+확장성+유용성 = "계획입니다"가 안 먹힘)은 완전히 다른 게임이다. 11월 3일 상세기획서 전까지는 플랫폼 활용·외부 협력·정확한 근거가, 그 이후엔 실제로 돌아가는 URL·실데이터·백테스트·사용자 테스트가 Point-per-Effort 1순위.

### 트랙: 서비스 개발 부문 (아이디어 부문 아님)

이전 README는 "아이디어/프로토타입 부문"으로 적혀 있었다 — 정정했다. 이유: 이미 구현된 자산(3D GIS·오케스트레이터·전국 행정경계 검색 등)이 아이디어 부문에 넣기엔 과하고, 아이디어 본선은 시장성 30점을 별도 평가하는데 재난 대응 B2G 서비스는 그 항목에 약하다. **9월 중순을 Go/No-Go 기준으로 둔다**: 그때까지 ① 산청 실데이터로 Module A가 최소 1건 처리, ② 공모전 지원 플랫폼 자원 신청·연동, ③ 외부 접속 가능한 배포 URL — 이 중 **최소 두 가지**를 확보 못 하면 아이디어 부문 재검토. 공모분야는 접수 후 변경 불가.

### 현재 상태 감사 (2026-08-28 기준, 보수적 추정)

| 관점 | 판정 |
|---|---|
| 문제의 중요성·서비스 서사·UI/시연 잠재력 | 강함 |
| 기술적 독창성 | 중상 — 개별 기능보다 통합 구조가 강점 |
| 핵심 모델 검증(Module A/B 실학습·산청 백테스트) | **약함** |
| 실제 사용자·외부 협력 증거 | **매우 약함** |
| 실제 배포 | 아직 미완료(Vercel/Render 준비는 됐으나 공모전 플랫폼 자원 미연동) |
| 예선 예상 점수(100점 만점, 보수적 추정) | **62점** (기획25·활용8·유용14·협력3·충실12) |
| 본선 예상 점수(오늘 당장 나간다면) | **47점** (기획12·완성도9·확장성13·유용성6·발표7) |

즉 지금 겉보기 완성도는 70~80처럼 보이지만, 심사표에 그대로 넣으면 40~60대가 나올 수 있다 — 화면이 예쁘고 핵심 출력이 mock이면 "완성도 25"의 정의(기술적·기능적 구현·배포)를 충족했다고 보기 어렵기 때문. 12월까지 실모델+산청 검증+실경로+플랫폼 배포+사용자 평가를 확보하면 내부 목표치로 본선 84/100 수준까지 설계 가능(공식 커트라인은 아님).
...
우선순위 재정렬(S/A/B/C 티어), MAWP(최소 수상가능 제품) 체크리스트, 본선 10분 발표 구조는 전부 [HANDOFF.md §9](HANDOFF.md)에 있다.
